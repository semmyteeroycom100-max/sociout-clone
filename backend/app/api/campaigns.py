...
def create_campaign(
    ...
    # ===== GAMIFICATION: XP for creating campaign =====
    add_xp(user, 5, db)
...
def start_campaign(
    ...
    # Run campaign logic
    try:
        result = _run_campaign_logic(campaign_id, user.id, db)
        return result
...
def _run_campaign_logic(campaign_id: int, user_id: int, db: Session):
    """
    Reusable campaign execution logic for scheduler and start endpoint.
    ...
    if success_count >= campaign.target_count:
        ...
        # ===== GAMIFICATION: XP for completing campaign =====
        add_xp(user, 20, db)
        update_streak(user, db)
        # Send webhook / email
    ...
    db.commit()
    return {"message": "Campaign processed", "completed": success_count, "target": campaign.target_count}
...
def export_campaign_csv(
    ...
    output.seek(0)
    return StreamingResponse(output, media_type="text/csv", headers={"Content-Disposition": f"attachment; filename=campaign_{campaign_id}.csv"})from fastapi import APIRouter, Depends, HTTPException, status, BackgroundTasks, Request
from fastapi.security import HTTPBearer, HTTPAuthorizationCredentials
from app.utils.rate_limit import limiter
from slowapi.errors import RateLimitExceeded
from fastapi.responses import StreamingResponse
